...
import com.example.bcawhatsapp.databinding.ActivityChatDetailBinding;
...
public class ChatDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final String senderId = auth.getUid();             //fetching data from onBindingViewHolder
        String recieveId = getIntent().getStringExtra("userId");
        String userName = getIntent().getStringExtra("userName");
        String profilePic = getIntent().getStringExtra("profilePic");

        binding.userName.setText(userName);               //displaying data
        Picasso.get().load(profilePic).placeholder(R.drawable.img_1).into(binding.profileImageChatDetail);
...
//                ChatDetailActivity.this.finish();                                //to finish activity**********************
        ...
        final ChatAdapter chatAdapter = new ChatAdapter(messageModels, this , recieveId);           //recieveId for the purpose of deleting message
        ...
        database.getReference().child("chats").child(senderRoom).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot snapshot1 : snapshot.getChildren()) {
                    ...
                    model.setMessageId(snapshot1.getKey());                  //id to delete the message
